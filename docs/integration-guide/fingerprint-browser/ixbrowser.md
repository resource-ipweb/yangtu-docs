...
Before configuring ixBrowser, prepare the following four proxy connection details:
...
After completing the steps above and obtaining the proxy server address, port, username, and password from 006IP, you can add the proxy to an ixBrowser browser profile.

### 1. Create a Browser Profile

Open **ixBrowser**, go to **Browser Profile → Profile List** from the left-hand menu, and then click **Create Profile** at the top of the page.

![Create an ixBrowser profile](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/ixbrowser-1-en.png)

On the profile creation page, switch to **Proxy Configuration**. Under **Proxy Method**, select **Custom** to enter the 006IP proxy information manually.

### 2. Enter the 006IP Proxy Information

Using the proxy details obtained from the 006IP dashboard in the previous section, enter the following information:
...
![Enter 006IP proxy information in ixBrowser](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/ixbrowser-2-en.png)

If you are using a static proxy, you generally do not need to enable **IP Detection** for every profile launch. If you want ixBrowser to recheck the proxy each time the profile starts, you can enable it as needed.
...
![Check the 006IP proxy in ixBrowser](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/ixbrowser-3-en.png)

If the check is successful, ixBrowser will display the current proxy egress IP address together with the corresponding country/region, state/province, city, and other information. This confirms that the proxy can connect successfully.

After the proxy check passes, click **Create** in the lower-right corner to finish creating the browser profile.

### 4. Launch the Browser Profile

After the profile is created, return to **Profile List**. Find the profile you just created and review the configured proxy information in the **Proxy** column.

Click **Open** on the right side of the profile to launch the browser.

![Launch the ixBrowser profile](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/ixbrowser-4-en.png)

Once the browser opens, network traffic from that profile will be routed through the configured 006IP proxy.

To further verify that the proxy is active, you can visit `https://ipinfo.io` or another IP lookup service from within the current ixBrowser profile and confirm that the displayed egress IP matches your 006IP proxy.

### 5. Troubleshooting

| Issue | What to Check |
| --- | --- |
| Proxy check fails | Verify that the proxy host and port are entered correctly. |
| Authentication failed | Recheck that the proxy username and password are complete and make sure no extra spaces were included when copying the credentials. |
| Proxy information is entered but the connection still fails | Confirm that the proxy type selected in ixBrowser matches the protocol used by the 006IP proxy. |
| The browser shows your local IP after launch | Return to **Proxy Configuration** and confirm that the custom proxy has been saved to the current profile. |
...
> Interface layouts and button labels may vary slightly between ixBrowser versions. Always follow the interface shown in your current client. Use proxies only for lawful and compliant business purposes, and comply with the target website's Terms of Service and all applicable local laws and regulations.
