...
Before configuring VMLogin, prepare the following four proxy connection details:
...
After completing the steps above and obtaining the **proxy IP address, port, username, and password** from 006IP, you can configure the proxy in **VMLogin**.

### 1. Create a New Browser Profile

Open **VMLogin** and click **New browser profile** on the left.

![Create a new VMLogin browser profile](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/vmlogin-1-en.png)

On the profile configuration page, under **Basic configuration**, click **Setting proxy server** to open the proxy settings window.

![Open VMLogin proxy settings](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/vmlogin-2-en.png)

### 2. Configure the 006IP Proxy

First, enable **Enable proxy server**, then enter the proxy information provided by 006IP:

- **Proxy type**: Select the proxy protocol that matches your 006IP proxy, such as `HTTP Proxy` or `SOCKS5 Proxy`.
...
![Enter 006IP proxy information in VMLogin](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/vmlogin-3-en.png)
...
The proxy protocol selected in VMLogin must match the protocol used when obtaining the proxy from 006IP.

### 3. Test the Proxy

After entering the proxy information, select a test site under **Test IP URL**, then click **Test Proxy**.

If the test is successful, VMLogin will display the current proxy egress IP address, along with the country, city, region, time zone, and other information.

![Test the 006IP proxy in VMLogin](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/vmlogin-4-en.png)

After confirming that the proxy connects successfully, click **Confirm** to close the test result, then click **Save**.

Return to the browser profile configuration page, complete any other settings as needed, and then click **Save profile**.

After saving, find the corresponding profile under **Browser profile** and launch it to use the configured 006IP proxy.
