...
Before configuring Incogniton, prepare the following four proxy connection details:
...
After completing the steps above and obtaining the proxy server address, port, username, and password from 006IP, you can add the proxy to an Incogniton browser profile.

### 1. Create a New Browser Profile

Open the **Incogniton** client, select **Profiles** from the left-hand menu, and then click **Create profile**.

![Create an Incogniton browser profile](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/incogniton-image1-en.png)

### 2. Enter the 006IP Proxy Information

On the profile creation page, open the **Proxy** tab and select **Custom proxy**. Then enter the proxy connection details obtained from the 006IP dashboard in the previous section.

| Incogniton Setting | What to Enter |
| --- | --- |
| **Connection type** | Select the connection type that matches the protocol of your current 006IP proxy. The example shown uses `HTTPS`. |
| **Proxy (ip:port)** | Enter the proxy host and port provided by 006IP in `HOST:PORT` format. |
...
```text
HOST:PORT → Incogniton "Proxy (ip:port)"
USERNAME  → Incogniton "Proxy username"
PASSWORD  → Incogniton "Proxy password"
```

![Enter 006IP proxy information in Incogniton](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/incogniton-image2-en.png)

> **Note:** The **Connection type** must match the actual proxy protocol provided by 006IP. If the wrong protocol is selected, the connection check may fail even if the host, port, username, and password are correct.

### 3. Check the Proxy Connection

After entering the proxy information, click **Check** at the bottom of the page.

If the result shows **Connected** and correctly displays the proxy's egress IP address, country/region, timezone, and other information, the 006IP proxy has been successfully connected to the current Incogniton browser profile.

![Check the 006IP proxy connection in Incogniton](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/incogniton-image3-en.png)

You can then verify that the detected egress IP matches the proxy currently being used in the 006IP dashboard. If the egress IP and target location match your expectations, continue configuring **Privacy & Network**, **Browser & Navigator**, **Hardware**, **Extensions & Cookies**, and other profile settings as needed.

### 4. Save and Launch the Browser Profile

After confirming that the proxy connection check passes, click **Create** in the lower-right corner to save the profile.

Once the profile has been created, launch it from the Incogniton profile list. Network requests from that browser profile will then be routed through the configured 006IP proxy when accessing target websites.

To further verify that the proxy is active, you can visit `https://ipinfo.io` or another IP lookup service from within the Incogniton browser profile and confirm that the displayed egress IP matches your 006IP proxy.

### 5. Troubleshooting

If the proxy check fails in Incogniton, check the following items in order:
...
| Credentials are entered but the proxy still does not work | Confirm that the **Connection type** selected in Incogniton matches the actual proxy protocol provided by 006IP. |
| The browser shows your local IP after launch | Return to the proxy settings for the current profile and confirm that the custom proxy was saved and successfully passed the connection check. |
...
> Interface layouts and button labels may vary slightly between Incogniton versions. Always follow the interface shown in your current client. Use proxies only for lawful and compliant business purposes, and comply with the target website's Terms of Service and all applicable local laws and regulations.
