...
Before configuring Hubstudio, prepare the following four proxy connection details:
...
After completing the steps above and obtaining the proxy server address, port, username, and password from 006IP, you can add the proxy to a Hubstudio browser profile.

### 1. Create a New Browser Profile

Open the **Hubstudio** client. On the **My Profiles** page, click **New Profile** to open the browser profile creation page.

![Create a new Hubstudio browser profile](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/hubstudio-image1.png)

Enter the profile name and other basic information based on your actual needs, then locate the **Proxy Information** section.

### 2. Configure the 006IP Proxy

In the proxy information section, set **Proxy Method** to **Custom**, then enter the proxy connection details obtained from the 006IP dashboard in the previous section.
...
| **Proxy Protocol** | Select the option that matches the protocol actually used by the current proxy. |
...
USERNAME  → Hubstudio "Proxy Username"
PASSWORD  → Hubstudio "Proxy Password"
```

![Enter 006IP proxy information in Hubstudio](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/hubstudio-image2.png)

> **Note:** The proxy type selected in Hubstudio must match the protocol actually provided by 006IP. If the wrong protocol is selected, the connection check may fail even if the server address, port, username, and password are correct.

### 3. Check the Proxy Connection

After entering the proxy information, click **Check Proxy**.

If the result shows **Connection test passed** and Hubstudio successfully returns the proxy egress IP address and corresponding country/region information, the 006IP proxy has been successfully connected to the current Hubstudio browser profile.

![Check the 006IP proxy in Hubstudio](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/hubstudio-image3.png)

You can then verify that the detected egress IP matches the proxy currently assigned in the 006IP dashboard. If the egress IP and target location match your expectations, continue configuring the remaining browser profile settings.

### 4. Save and Launch the Browser Profile

After the proxy passes the connection check, complete the remaining browser profile settings and click **Finish** to save the profile.

Return to the **My Profiles** page, find the profile you just created, and click **Open**. Once launched, network traffic from that browser profile will be routed through the configured 006IP proxy when accessing target websites.

To further verify that the proxy is active, you can visit `https://ipinfo.io` or another IP lookup service from within the current Hubstudio browser profile and confirm that the displayed egress IP matches your 006IP proxy.

### 5. Troubleshooting

If the proxy check fails in Hubstudio, check the following items in order:
...
| Credentials are entered but the proxy still does not work | Confirm that the proxy type selected in Hubstudio matches the actual protocol of the 006IP proxy. |
| The browser shows your local IP after launch | Return to the proxy information for the current profile and confirm that the custom proxy was saved and successfully passed **Check Proxy**. |
...
> Interface layouts and button labels may vary slightly between Hubstudio versions. Always follow the interface shown in your current client. Use proxies only for lawful and compliant business purposes, and comply with the target website's Terms of Service and all applicable local laws and regulations.
