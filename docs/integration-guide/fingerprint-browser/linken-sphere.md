...
Before configuring Linken Sphere, prepare the following four proxy connection details:
...
After completing the steps above and obtaining the **proxy IP address, port, username, and password** from 006IP, you can configure the proxy in **Linken Sphere**.
...
After the session is created, it will appear in the session list on the current Desktop.

![Create a browser session](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/linkensphere-1.png)

### 2. Configure the 006IP Proxy

In the session list, find the session you just created and click the **⋮** menu on the far right.

![Configure the 006IP proxy](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/linkensphere-2.png)

From the expanded menu, select **Set proxy**.

On the **Set proxy to** page, you can add a new proxy or select an existing one for the current session.

![Open the proxy settings](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/linkensphere-3.png)

If you have not added a proxy yet, click **NEW PROXY** and enter the proxy information obtained from 006IP, including:

- **Proxy protocol**: Select the same protocol used when obtaining the proxy from 006IP, such as `SOCKS5`, `HTTP`, or `HTTPS`.
...
![Enter 006IP proxy information](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/linkensphere-4.png)

After entering the proxy information, save the proxy. Once saved, it will appear in the proxy list together with the detected country/region, city, time zone, and other information.

![View the saved proxy](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/linkensphere-5.png)

If you need to add multiple 006IP proxies at once, you can also use **IMPORT FROM LIST** in the upper-right corner to import a proxy list in bulk.

### 3. Check the Proxy

In the proxy list, select the 006IP proxy you just added, then click **CHECK PROXY** at the bottom.

When the page displays **Connection successfully validated**, the proxy is connecting properly.

![Check the 006IP proxy](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/linkensphere-6.png)
...
Once the proxy has been bound successfully, click the launch button on the left side of the session to start the browser.

After the browser launches, the session will connect to the network through the configured 006IP proxy. If you need to switch proxies later, open the **⋮** menu on the right side of the session again, select **Set proxy**, and choose or add another proxy.
