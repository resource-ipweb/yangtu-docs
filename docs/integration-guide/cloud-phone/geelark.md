---
sidebar_position: 2
sidebar_label: GeeLark Cloud Phone Guide
description: Complete guide to configuring 006ip static residential proxies in GeeLark Cloud Phone.
---

# GeeLark Cloud Phone — 006IP Proxy Guide
...
Before configuring GeeLark Cloud Phone, prepare the following four proxy connection details:
...
## Configure GeeLark Cloud Phone

After completing the steps above and obtaining the **proxy IP address, port, username, and password** from 006IP, you can configure the proxy in a GeeLark Cloud Phone profile.
...
enter `HOST`, `PORT`, `USERNAME`, and `PASSWORD` into the corresponding **Host**, **Port**, **Username**, and **Password** fields in GeeLark. The steps below cover creating a Cloud Phone profile, entering the proxy details, checking the connection, and launching the profile.

### 1. Create a New Cloud Phone Profile

Open **GeeLark**, go to **Profiles** from the left-hand menu, and click **New profile** in the upper-right corner.

![Create a new GeeLark Cloud Phone profile](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/geelark-cloudphone-new-profile-create-en.png)

On the **New profile** page, select the **Cloud Phone** device type at the top and use **Single** mode.

Under **Profile settings**, enter or select the profile information as needed, such as:

- **Name**: Enter a profile name that is easy to identify.
- **Group**: Select or create a group as needed.
- **Platform**: Select the platform based on your actual use case.
- **Tags**: Add tags if needed.
- **Operating system**: Select Android or iOS as required.

![Configure GeeLark Cloud Phone profile settings](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/geelark-cloudphone-profile-settings-en.png)

### 2. Configure the 006IP Proxy

Scroll down to **Proxy settings** and set **Option** to **Custom**.

This guide uses SOCKS5 as an example. Enter the proxy information as follows:

| GeeLark Setting | 006IP Information |
| --- | --- |
| **Type** | `Socks5` |
| **IP query channel** | `IP2Location` |
| **Host** | Enter the proxy IP address/host provided by 006IP. |
| **Port** | Enter the proxy port provided by 006IP. |
| **Username** | Enter the proxy username provided by 006IP. |
| **Password** | Enter the proxy password provided by 006IP. |

Enter the proxy details obtained from 006IP into the corresponding fields.

![Enter the 006IP SOCKS5 proxy in GeeLark](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/geelark-cloudphone-proxy-settings-en.png)

The **Change IP URL** field is optional. If the current 006IP proxy does not provide a corresponding Change IP URL, leave it blank. **Auto refresh** can also be configured based on how the current proxy is intended to be used.

### 3. Check the Proxy Connection

After entering the proxy information, click **Check proxy**.

If the check is successful, GeeLark will display information about the current proxy in the proxy settings section, such as:
...
Confirm that the detected information matches the 006IP proxy you are currently using, then continue creating the profile.

![Check the 006IP proxy connection in GeeLark](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/geelark-cloudphone-proxy-check-en.png)

### 4. Create and Launch the Profile

After configuring and checking the proxy, click **Create** at the bottom of the page.

Once the profile is created, return to the **Profiles** page. Find the Cloud Phone profile you just created and click **Start** on the profile card to launch the GeeLark Cloud Phone environment with the configured 006IP proxy.

![Start the GeeLark Cloud Phone profile](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/geelark-cloudphone-start-en.png)
