---
sidebar_position: 5
sidebar_label: MostLogin Cloud Phone Guide
description: Complete guide to configuring 006ip static residential proxies in MostLogin Cloud Phone.
---

# MostLogin Cloud Phone — 006IP Proxy Guide
...
Before configuring MostLogin Cloud Phone, prepare the following four proxy connection details:
...
## Configure MostLogin Cloud Phone

After completing the steps above and obtaining the **proxy IP address, port, username, and password** from 006IP, you can configure the proxy in a MostLogin Cloud Phone environment.
...
enter `HOST`, `PORT`, `USERNAME`, and `PASSWORD` into the corresponding **Host:Port**, **Account**, and **Password** fields in MostLogin. The steps below follow the current MostLogin interface to complete the setup, proxy check, and launch process.

### 1. Create a New Cloud Phone Environment

Open **MostLogin**, go to the Cloud Phone environment page, and click **New env** at the top.

![Create a new MostLogin Cloud Phone environment](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/mostlogin-cloud-phone-newenv-create-en.png)

On the **Basic Information** page, enter the environment details:

- **Title**: Enter a name that is easy to identify.
- **Group**: Select the group where you want to save the environment.
- **Remark**: Add a note if needed.

Under **Cloud phone devices**, choose either **Owned devices** or **Temporary device**. If you use a temporary device, select the Android version you need, then click **Next**.

![Configure the basic information for a MostLogin Cloud Phone environment](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/mostlogin-cloud-phone-basic-info-settings-en.png)

### 2. Configure the 006IP Proxy

On the **Advanced Settings** page, under **Proxy Settings**, select **Set up now**.

This guide uses a SOCKS5 proxy as an example. Enter the proxy information obtained from 006IP as follows:

| MostLogin Setting | 006IP Information |
| --- | --- |
| **Proxy protocol** | `socks5` |
| **Host:Port** | The proxy IP / host and port provided by 006IP |
| **Account** | The proxy username provided by 006IP |
| **Password** | The proxy password provided by 006IP |

![Enter the 006IP SOCKS5 proxy information in MostLogin](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/mostlogin-cloud-phone-proxy-settings-en.png)

MostLogin indicates that a custom proxy applies only to the current environment. After confirming that the host, port, account, and password are correct, continue to the proxy check.

### 3. Check the Proxy Server IP

Click **Check proxy server IP**.

If the check is successful, MostLogin will display information about the current proxy, including:
...
Confirm that the detected information matches the 006IP proxy node you are currently using.

![Check the 006IP proxy connection in MostLogin](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/mostlogin-cloud-phone-proxy-check-en.png)

If you need UDP support, you can keep **UDP Protocol** enabled. After confirming that the proxy check is successful, click **Confirm** in the lower-right corner to finish creating the environment.

### 4. Start the Cloud Phone

After the environment is created, return to the environment list. You will see the newly created Cloud Phone environment.

Click **START** in the **Ops** column to launch the MostLogin Cloud Phone that has already been configured with the 006IP proxy.

![Start the MostLogin Cloud Phone configured with a 006IP proxy](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/mostlogin-cloud-phone-start-en.png)
